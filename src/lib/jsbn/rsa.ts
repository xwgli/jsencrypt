...
function pkcs1pad1ForPrivateKeyEncrypt(s:string, n:number) {
    if (n < s.length + 22) {
        console.error("Message too long for RSA");
        return null;
    }
...
export class RSAKey {
  constructor() {
    ...
    // Return the PKCS#1 RSA encryption of "text" as an even-length hex string
    public encryptByPrivate(text:string) {
        const maxLength = (this.n.bitLength() + 7) >> 3;
        const m = pkcs1pad1ForPrivateKeyEncrypt(text, maxLength);
    ...
    // Return the PKCS#1 RSA decryption of "ctext".
    // "ctext" is an even-length hex string and the output is a plain string.
    public decryptByPublic(ctext:string) {
        const c = parseBigInt(ctext, 16);
...
// Undo PKCS#1 (type 2, random) padding and, if valid, return the plaintext
function pkcs1unpad2ForPublicKeyDecrypt(d:BigInteger, n:number):string {
    const b = d.toByteArray();
    let i = 0;
    while (i < b.length && b[i] == 0) { ++i; }
